...
// V 1.9            SC (TCS)               R2012.1 - CAFR - suppression mention cheque kdo
// V 1.10           Teesta (TCS)           R2012.1 - CAFR - Nouveau_moyen_de_paiement
// V 1.11             Chayanika(TCS)         R2012.1  Regression Fix 
...
resourcestring
  ...
  CtTxtAppTitle    = 'Reprint the count results';    //R2012.1  Regression Fix (Chayanika)
...
uses
  ...
  DFpnTenderGroupCA;            //Nouveau_moyen_de_paiement,R2012.1 - CAFR,Teesta
...
const  // PVCS-keywords
  ...
  CtTxtSrcDate    = '$Date: 2012/04/03 06:55:59 $';
...
begin  // of TFrmDetTndReprint.Initialize
  ...
  //if FlgRemoveChequeKDO then  begin                                          //TCS Modification R2012.1--CAFR-suppression mention cheque kdo (sc)
   //DmdFpnTenderGroupCA.BuildListTenderGroupCAFR(LstTenderGroup)              //TCS Modification R2012.1--CAFR-suppression mention cheque kdo (sc)
  //end
  //else
    ...
    if ChkLbxOperator.Checked [CntChkLstBx] then begin
      ...
      if not FlgVerkTotAdded then begin
        IdtSafeTrans := DmdFpnSafeTransactionCA
                                .RetrieveRunningSafeTransOperator (IdtOperator);
        // Build the safetransaction list for specified transaction
        LstSafeTransaction.ClearSafeTransactions;
        DmdFpnSafeTransaction.BuildListSafeTransactionAndDetail (
                                      LstSafeTransaction,
...
begin  // of TFrmDetTndReprint.FormCreate
  inherited;
  Application.Title := CtTxtAppTitle;                                  //2012.1 Regression Fix (Chayanika)
