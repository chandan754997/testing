...
// Customer : Castorama
//-----------------------------------------------------------------------------
// CVS      : $Header: /development/castorama/Flexpoint/Src/Flexpoint201/Develop/FVSRptInvoiceCA.pas,v 1.46 2010/07/05 15:35:08 BEL\DVanpouc Exp $
// History  :
...
uses
  ...
  ExtCtrls,ToolWin, MMntInvoice, FVSRptInvoice;
...
var    // Column-width (in number of characters)
  ViValRptWidthDescr    : Integer = 24;// Width columns description
  ...
  ViValRptWidthPayDescr : Integer = 24;// Width columns Description payment
  ...
  ViValRptTotRectWidth  : Integer = 2640;// Width of Payment table
  ViValRptPayRectWidth  : Integer = 3300;// Width of Payment table

  ViValRptInvoiceRuPosY : Integer = 2300;   // Y position of Invoice info RU
  ViValRptVATPosY       : Integer = 11800;  // Y position of VAT table
  ViValRptPaymentPosX   : Integer = 4750;   // X position of Payment table
  ...
  ViValRptBodyPoxY      : Integer = 5650;   // Y position of Body table
  ...
  ViValRptClientIntracomWidth : Integer = 3000;  // Width of remarque box
  ViValRptClientIntracomHeight: Integer = 500;   // Height of remarque box
...
const  // PVCS-keywords
  ...
  CtTxtSrcVersion = '$Revision: 1.46 $';
  CtTxtSrcDate    = '$Date: 2010/07/05 15:35:08 $';
...
begin  // of TFrmVSRptInvoiceCA.PrintGeneralInvoiceInfo
  ...
  if CodKind = CodKindDelivNote then begin
    ...
    if (not FlgFiscal) and (not FlgRussia) then
      VspPreview.Text := CtTxtTicketDate + ' : ' + TxtAllTickDates + #10;
    if not FlgRussia then begin
      if FlgCreditnote then
...
begin  // of TFrmVSRptInvoiceCA.GetTxtRemarqueTable
  ...
  if FlgCreditStrip and FlgClientEnCompte and not FlgCreditnote then         
